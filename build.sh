...
#   - APP_COMMIT: App commit hash
#   - APP_PLATFORMS: Platforms to build for (e.g. "windows/amd64,linux/amd64,darwin/amd64")
...
make_ldflags() {
    ...
    if [ -n "$APP_COMMIT" ]; then
        ldflags="$ldflags -X 'main.appCommit=$APP_COMMIT'"
    else
...
build_for_platform() {
    ...
    local output="build/aclproxy-$GOOS-$GOARCH"
...
if [ -z "$APP_PLATFORMS" ]; then
    APP_PLATFORMS="$(go env GOOS)/$(go env GOARCH)"
fi
